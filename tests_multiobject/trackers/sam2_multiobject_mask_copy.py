...
class NCCTracker2(object):
    ...
    def __init__(self, image, mask, cnt):
        ...
        #print("LEN", len(points), len(points[np.random.choice(points.shape[0], 5, replace=False)]))
        # print(points[np.random.choice(points.shape[0], 5, replace=False)])
        # print(points[:5])

        # points = points[150:200]

        # # print(points)

        # # # # Randomly select 3 points
        points = points[np.random.choice(points.shape[0], 55, replace=False)]
        points = points[:, ::-1]
        ...
        #self.vis_segm(image, (out_mask_logits[0] > 0.0).cpu().numpy().astype(np.uint8)[0], "/home.stud/rozumrus/BP/tests_multiobject/out_sam2/" + str(cnt) + "first.png")
        ...
        for i in range(2, rows - 2):
            for j in range(1, cols - 2):
                # Check if the current point is 1 and has no neighboring 0's
                 ...
                 #        binary_mask[i-1, j] == 1 and  # up
                 #        binary_mask[i+1, j] == 1 and  # down
                 #        binary_mask[i, j-1] == 1 and  # left
                 #        binary_mask[i, j+1] == 1 and  # right
                 #        binary_mask[i-1, j-1] == 1 and  # top-left
                 #        binary_mask[i-1, j+1] == 1 and  # top-right
                 #        binary_mask[i+1, j-1] == 1 and  # bottom-left
                 #        binary_mask[i+1, j+1] == 1):    # bottom-right
                        
                 #        valid_indices.append([i, j])

                for k in range(1, 3):
                    if (binary_mask[i, j] == 1 and
                        binary_mask[i-k, j] == 1 and  # up
                        binary_mask[i+k, j] == 1 and  # down
                        binary_mask[i, j-k] == 1 and  # left
                        binary_mask[i, j+k] == 1 and  # right
                        binary_mask[i-k, j-k] == 1 and  # top-left
                        binary_mask[i-k, j+k] == 1 and  # top-right
                        binary_mask[i+k, j-k] == 1 and  # bottom-left
                        binary_mask[i+k, j+k] == 1):    # bottom-right
