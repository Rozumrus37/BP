...
def get_mask(prev_frame_path, next_frame_path, prev_segmentation_mask, frame_idx=None, seq=None, direct_comp_to_prev_pred=False, 
    ...
    model = ptlflow.get_model('sea_raft_m', 'spring').to(device)
    # model = ptlflow.get_model('raft', 'sintel').to(device)
    model.eval()
    ...
    predictions = model({'images':inputs['images'].to(device)})#inputs)
...
#     # prev_segmentation_mask = np.zeros((H_original, W_original), dtype=np.uint8)

#     # prev_segmentation_mask[30:40, 30:40] = 1
